        ...
        <form className="admin-login-form animate-fade-in" onSubmit={handleLogin} style={{ background: 'rgba(255,255,255,0.02)', backdropFilter: 'blur(20px)', border: '1px solid rgba(255,255,255,0.05)' }}>
          ...
          <div style={{ textAlign: 'right', marginBottom: '1.5rem' }}>
            <button 
              ...
              style={{ background: 'none', border: 'none', color: 'rgba(255,255,255,0.4)', fontSize: '0.8rem', cursor: 'pointer', fontWeight: '600' }}
            >
              Forgot Password?
            </button>
          </div>
                      ...
                      <tr key={reg.id} style={{ background: 'rgba(230,184,0,0.02)' }}>
                        ...
                        <td>
                          <span className="badge" style={{ border: '1px solid rgba(255,255,255,0.2)' }}>
                            {(reg.businessCategory || reg.category || 'N/A').toUpperCase()}
                          </span>
                        </td>
                        <td>
                          <span className="badge badge-paid">
                            {(reg.stallSize || 'Standard').toUpperCase()}
                          </span>
                        </td>
                        <td>
                          <span className={`status-dot ${reg.status}`}></span>
          ...
          <div className="admin-section animate-fade-in">
            <div className="admin-card" style={{ maxWidth: '1000px', margin: '0 auto', background: 'var(--color-bg-panel)', padding: '2.5rem', borderRadius: '16px', border: '1px solid var(--color-border)' }}>
              ...
              <form onSubmit={handleSaveOrgData}>
                ...
                <div className="section-header" style={{ borderBottom: '1px solid var(--color-border)', paddingBottom: '1rem', marginBottom: '1.5rem' }}>
                  <h3 style={{ fontSize: '1.1rem', color: 'var(--color-text-primary)' }}>Homepage Statistics Override</h3>
                </div>

                <div className="form-grid" style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr 1fr', gap: '1.5rem', marginBottom: '3rem' }}>
                  <div className="form-group">
                    <label style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>Visitors (e.g. 15k+)</label>
                    <input type="text" className="input-field" value={orgData.stats?.visitors || ''} onChange={e => setOrgData({...orgData, stats: {...orgData.stats, visitors: e.target.value}})} />
                  </div>
                  <div className="form-group">
                    <label style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>Vendors (e.g. 500+)</label>
                    <input type="text" className="input-field" value={orgData.stats?.vendors || ''} onChange={e => setOrgData({...orgData, stats: {...orgData.stats, vendors: e.target.value}})} />
                  </div>
                  <div className="form-group">
                    <label style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>Cities (e.g. 50+)</label>
                    <input type="text" className="input-field" value={orgData.stats?.cities || ''} onChange={e => setOrgData({...orgData, stats: {...orgData.stats, cities: e.target.value}})} />
                  </div>
                  <div className="form-group">
                    <label style={{ fontSize: '0.8rem', color: 'var(--color-text-secondary)' }}>Success Rate (e.g. 98%)</label>
                    <input type="text" className="input-field" value={orgData.stats?.success || ''} onChange={e => setOrgData({...orgData, stats: {...orgData.stats, success: e.target.value}})} />
                  </div>
                </div>
